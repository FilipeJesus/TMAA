...
genalg3 = function (sizeX = 10,sizeY = 10, suggestions = NULL, popSize = 200, iters = 100,
        ...
        if (varsX > 1) {  #varsX>1 and varsY >1??
          ...
          for (child in (elitism + 1):popSize) {
            ...
            if (crossOverPointX == 0) {
              ...
              childX = c(parents[1,1:varsX ][1:crossOverPointX], parents[2,1:varsX ][(crossOverPointX + 1):varsX])
            }
            ...
            if (crossOverPointY == 0) {
              ...
              childY = c(parents[1,(varsX+1):(varsX+varsY) ][1:crossOverPointY], parents[2,(varsX+1):(varsX+varsY) ][(crossOverPointY + 1):varsY])
            }
            while(sum(childX)==0){
		          childX = sample(c(rep(0, zeroToOneRatioX), 1), varsX, rep = TRUE)
		        }
	          while(sum(childY)==0){
	            childY = sample(c(rep(0, zeroToOneRatioY), 1), varsY, rep = TRUE)
	          }
          ...
          for (object in (elitism + 1):popSize) {
            for (var in 1:varsX) {
              if (runif(1) < mutationChanceX) {
                population[object, var] = sample(c(rep(0,zeroToOneRatioX), 1), 1)
